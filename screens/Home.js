...
export default ({ navigation }) => {
    ...
    const listsRef = auth.currentUser
        ? collection(doc(firestore, "users", auth.currentUser.uid), "lists")
        : null;

    useEffect(() => {
        if (!auth.currentUser) {
            navigation.replace("Login"); // Arahkan ke layar login jika belum login
            return;
        }

        const unsubscribe = listsRef
            ? onSnapshot(
                  listsRef,
                  (newLists) => {
                      setLists(newLists);
